...
const Diensten = () => {
    ...
    const contentStyles = {
        ...
        maxHeight: isExpanded ? '10000px' : '750px'  // 10000px is an arbitrary large number to ensure all content is shown when expanded
    };
    const wrapperStyles = {
        position: 'relative',
    };
    ...
    const gradientStyles = {
        ...
        pointerEvents: 'none',  // ensures the gradient doesn't interfere with interactions
    };
    ...
    return (
        <section className={` ${styles.gutter} ${styles.mdHorizontalCenter} py-16 md:py-32`}>
            ...
            <div className=" flex gap-24">
                ...
                <div className='max-w-[260px] w-full flex flex-col p-7 gap-6 bg-secondary-100 rounded-3xl box-content h-fit'>
                    ...
                    {seoDiensten.dienten.map((dienst, index) => (
                        <div
                            className={`bg-secondary-50 px-12 py-4 text-lg leading-7 font-medium text-center rounded-lg
                         ${activeDienstIndex === index
                                    ? "text-indigo-500 border border-1 border-[#9869E9] test"
                                    : "text-secondary-600 shadow-sm"
                                }`}
                            key={index}
                ...
                <div className='flex-grow'>
                    {seoDiensten.dienten.map((diensten, index) => (
                        <div key={index}
                            className={` ${activeDienstIndex === index ? "flex flex-col" : "hidden"
                                }`}>
                            ...
                            <div className={`content-wrapper ${isExpanded ? 'expanded' : ''}`} style={wrapperStyles}>
                ...
                <div className='max-w-[236px] w-full flex-none'>
                    {seoDiensten.rightSide.icon}
                    {seoDiensten.rightSide.body}
                </div>
            </div>
        </section>
    )
...
const seoDiensten = {
    ...
    dienten: [

        {
            ...
            <h3>Gepersonaliseerd Actieplan</h3>
            <p>Na een grondige analyse stellen we een gedetailleerd rapport op met specifieke aanbevelingen. Dit rapport dient als uw gepersonaliseerde actieplan om de SEO-prestaties van uw website te verbeteren. We bieden heldere, stapsgewijze instructies die u kunt volgen om de voorgestelde verbeteringen aan te brengen. Of het nu gaat om technische optimalisatie, het versterken van uw contentstrategie of het cultiveren van waardevolle backlinks, wij staan klaar om u door dit proces te leiden.</p>
            `
        },
        {
